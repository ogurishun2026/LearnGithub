...
def main():
    ...
    bpy.ops.import_anim.bvh(filepath=str(out / 'ARDY_Walk_Core27.bvh'), global_scale=1,
                            frame_start=1, rotate_mode='QUATERNION', use_fps_scale=False,
                            update_scene_fps=True, update_scene_duration=True,
                            axis_forward='-Z', axis_up='Y')
    # Blender's BVH duration update adds an inclusive endpoint; this clip has 40 samples.
    scene.frame_start, scene.frame_end = 1, 40
    ...
    action = bpy.data.actions.new('ARDY_Walk_RootMotion_v1')
    ...
    report = {'blender_version': bpy.app.version_string, 'bones': len(arm.data.bones),
              ...
              'frame_range': [1, 40], 'fps': 20, 'unit_scale_m': 0.01,
    ...
    export(out / 'AN_ARDY_Walk_RootMotion.fbx', True, True)
    ...
    bpy.ops.mesh.primitive_plane_add(size=1200, location=(0, 0, -0.5))
    ...
    camera.rotation_euler = (Vector((-25, -65, 90)) - camera.location).to_track_quat('-Z', 'Y').to_euler()
    ...
    for area in bpy.context.screen.areas:
        if area.type == 'VIEW_3D':
            ...
            area.spaces.active.region_3d.view_location = Vector((-25, -60, 100))
    scene.frame_set(1)
    bpy.ops.wm.save_as_mainfile(filepath=str(out / 'ARDY_Walk_Core28.blend'))
    for frame in (1, 11, 21, 31, 40):
        scene.frame_set(frame)
