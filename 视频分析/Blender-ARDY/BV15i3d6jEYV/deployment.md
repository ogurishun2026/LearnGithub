...
本次关闭动作后处理，未验收脚滑、长序列稳定性、6 GB 显卡速度、完整交互控制、视频作者的第三方 BVH 扩展、Blender 目标角色重定向或 TensorRT。后续已用自制脚本完成本样本的 BVH 导入、Blender 工程及 FBX 回读验证，文件和步骤见 [Blender → UE 指南](blender-ue/README.md)；UE 导入/重定向尚未实测。所附小样本和画面可用于验证部署链路，不能代表生产动画质量。代码、模型与量化模型许可分别见上游项目页面。
