...
def main():
    ...
    # In-place combat idle: pelvis owns the small body sway; independent root stays fixed.
    root_translation = np.zeros_like(positions[:, 0])
    ...
    write_bvh(args.out / 'ARDY_CombatIdle_Core27.bvh', names[1:], skeleton.joint_parents.tolist(),
    ...
    report = {'source_sha256': hashlib.sha256(args.motion.read_bytes()).hexdigest(),
              ...
              'changes': 'Added fixed root for in-place idle; renamed Hips to pelvis; source loop world joint positions/rotations preserved',
