...
def main():
    ...
    bpy.ops.import_anim.bvh(filepath=str(out / f'{clip_name}.bvh'), global_scale=1,
    ...
    if weights.shape[1] > 4:
        weights[np.arange(len(weights))[:, None], order[:, :-4]] = 0
    ...
    if max_skin_delta > 0.01:
        raise RuntimeError(f'Reference skin mismatch exceeds 1 cm: {max_skin_delta}')
    ...
    export(out / f'AN_{clip_name}.fbx', True, True)

    # Review scene only; camera and floor are excluded from both FBX exports.
    center = Vector((C @ ref['positions'][:, 1].mean(axis=0) * 100).tolist())
    center.z = 95
    extent = np.ptp(ref['positions'][:, 1], axis=0).max() * 100
    ...
    bpy.ops.object.camera_add(location=center + Vector((330, -490, 135)))
    camera = bpy.context.object
    camera.rotation_euler = (center - camera.location).to_track_quat('-Z', 'Y').to_euler()
    camera.data.type = 'ORTHO'
    camera.data.ortho_scale = max(245, 210 + extent)
    ...
    # Drop playback's duplicate endpoint only when the input contract explicitly marks a loop.
    scene.frame_end = contract['playback_end']
    scene.frame_set(1)
    bpy.ops.wm.save_as_mainfile(filepath=str(out / f'{clip_name}.blend'))
    for frame in samples:
        scene.frame_set(frame)
